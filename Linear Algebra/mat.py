...
def setitem(M, k, val):
    ...
    True
    """
    # Make sure both keys are in the domains and return if not
    if k[0] not in M.D[0] or k[1] not in M.D[1]:
        return
...
def add(A, B):
    ...
    for key1 in A.D[0]:
        for key2 in A.D[1]:
            outDict[(key1, key2)] = A[(key1, key2)] + B[(key1, key2)]

    return Mat(A.D, outDict)
...
def transpose(M):
    ...
    for row in M.D[0]:
        for col in M.D[1]:
            outDict[ (col, row) ] = M[ (row, col) ]

    return Mat( (M.D[1], M.D[0]) , outDict)
...
def vector_matrix_mul(v, M):
    ...
    """
    assert M.D[0] == v.D
    return Vec(M.D[1], {col : sum([v[row] * M[row, col] for row in v.D]) for col in M.D[1]})
...
def matrix_vector_mul(M, v):
    ...
    """
    assert M.D[1] == v.D
    return Vec(M.D[0], {row : sum([v[col] * M[row, col] for col in v.D]) for row in M.D[0]})
...
def matrix_matrix_mul(A, B):
    ...
    """
    assert A.D[1] == B.D[0]
    return Mat( (A.D[0], B.D[1]), {(row, col) : sum([ A[row, index] * B[index, col] for index in A.D[1] ]) for row in A.D[0] for col in B.D[1]})
